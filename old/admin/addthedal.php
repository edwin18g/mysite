...
var y = document.forms["myForm"]["churchid"].value;
	...
	{
	$count=1;
	$sql = "SELECT max(id) as id FROM thedal";
...
$title=$_POST['title'];
 ...
 if(move_uploaded_file($_FILES["photo"]["tmp_name"],"../thedal/".$newFileName))
                  {
			$sql = "insert into thedal(ctitle,cfilename) values('$title','$newFileName')"; 

	$val=$conn->query($sql);
	if($val)
	{
	header("Location:viewthedal.php?msg=2");
	exit();
	}
 ...
 <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Add Thedal</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
...
<input type="submit" value="Add thedal" class="btn btn-success" name="insert"/>
</div>
<div class="col-md-6">
