...
var y = document.forms["myForm"]["churchid"].value;
	...
	{
	$count=1;
	$sql = "SELECT max(id) as id FROM newsletter";
...
$title=$_POST['title'];
 ...
 if(move_uploaded_file($_FILES["photo"]["tmp_name"],"../newsletter/".$newFileName))
                  {
			$sql = "insert into newsletter(ctitle,cfilename) values('$title','$newFileName')"; 

	$val=$conn->query($sql);
	if($val)
	{
	header("Location:viewnewsletter.php?msg=2");
	exit();
	}
...
<input type="submit" value="Add Newsletter" class="btn btn-success" name="insert"/>
</div>
<div class="col-md-6">
