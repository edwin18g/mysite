...
$pname=mysqli_real_escape_string($conn,$_POST['pname']);
	...
	$pemail=$_POST['pemail'];
	$phone=$_POST['phone'];
$contract=$_POST['status'];
	$sql = "update odiocesepriest set  priestid='$priestid',contract='$contract',pname='$pname',pfname='$pfname',pmnane='$pmnane',pdob='$pdob',pplace='$pplace',pseminary='$pseminary',porddate='$porddate',pordplace='$pordplace',pordby='$pordby',pminstring='$pminstring',paddress='$paddress',pemail='$pemail',pphone='$phone' where id=".$cid;

	$val=$conn->query($sql);
	if($val)
	{
	header("Location:viewotherdiocesepriest.php?msg=1");
	exit();
	}
	}
	...
	$id=$_GET['id'];
	$sql = "SELECT * FROM odiocesepriest where id=".$id;
...
if($row['contract']=='yes')
{
 ...
 echo "<option selected=true>yes</option>";
}
else
{
echo "<option selected=true>no</option>";
