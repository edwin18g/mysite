...
{
 		if(move_uploaded_file($_FILES["photo"]["tmp_name"],"../newsletter/".$newFileName))
                  {
			$sql = "update newsletter set ctitle='$title',cfilename='$newFileName' where id='$id'"; 

			$val=$conn->query($sql);
			if($val)
			{
			header("Location:viewnewsletter.php?msg=1");
			exit();
			}
...
{
$sql = "update newsletter set ctitle='$title' where id='$id'"; 
...
$row1 = $result->fetch_assoc()
 ...
 <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Update Newsletter</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
...
<input type="submit" value="Update Newsletter" class="btn btn-success" name="insert"/>
</div>
<div class="col-md-6">
